...
public class MqttAndroidClient extends BroadcastReceiver implements
	...
	private void connectExtendedAction(Bundle data){
		...
		if(callback instanceof MqttCallbackExtended){
			...
			if(status == Status.INTERMEDIATE){
				int reconnectDelay = data.getInt(MqttServiceConstants.CALLBACK_RECONNECT_DELAY);
				((MqttCallbackExtended) callback).startingConnect(reconnect);
			}
			else {
	...
	private void simpleAction(IMqttToken token, Bundle data) {
		if (token != null) {
			...
			else if(status == Status.INTERMEDIATE){
				((MqttTokenAndroid) token).notifyIntermediate();
			}
			else {
	...
	public int getReconnectDelay(){
		return mqttService.getConnection(clientHandle)!=null?mqttService.getConnection(clientHandle).getReconnectDelay():0;
	}
